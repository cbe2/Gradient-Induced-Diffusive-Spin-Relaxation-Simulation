...
Lscale=np.sqrt((2*np.pi/np.radians(tipAngle))**2-1)*B1/G
pos=np.linspace(-3*Lscale,3*Lscale,100) #cm
thetas=[]
expThetas=[]
phases=[]
expPhases=[]
...
for x in pos:
    ...
    aphase=oD*times[-1]/2
    rePart=np.cos(aphase)*np.sinc(aphase/np.pi)*np.sin(o1*T)
    imPart=np.sin(aphase)*np.sinc(aphase/np.pi)*np.sin(o1*T)
    Xpts.append(rePart)
    Ypts.append(imPart)
